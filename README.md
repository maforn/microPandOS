...
The policy we use for scheduling is to use a round robin that will be called every 5 milliseconds: it will put the current process at the end of the ready queue and call the next one. Non-blocking syscalls will not call the scheduler again, blocking syscalls will.   

---
### The Exception Handler
When an [exception](/phase2/exceptions.c) is raised, the current process's state will be stored on the BIOSDATAPAGE address and the function `exceptionHandler` will be called. Based on the type of exception different functions may be called. If the exception causes it a trap, an illegal syscall, or a TLB exception the process will be killed (if it does not have a support struct) or the exception will be passed up with a vector to the Support Level (phase 3).  
...
#### Receive Message
This function behaves like the asynchronous receive of the message-passing paradigm. If there are already messages in the current process's inbox it will pop the first, else it will block the process (if it is not already blocked in another queue) and call the scheduler.  
...
When the time slice allocated to the current process finishes an interrupt is fired, at this point, we update the current process's status and put it at the end of the ready queue. Then we call the scheduler.  
...
We have added a new camp to the original structure of `pcb_t`, as we deemed it necessary to have an additional `blocked` camp to check if the process was already in a queue of blocked pcbs or not. This is necessary because each process can be waiting for both a message and the Interval Timer or a Device IO. As each process `p_list` cannot be in two queues at the same time, we manage the insertion in each queue with this var.  
Let's take as a case study the doIO operation. The process sends a request to the SSI and then does a syscall to receive the SSI's answer. If the local timer sends an interrupt before the process manages to ask to do the blocking syscall then it will be put by the `doIO` in the list for the PCBs waiting for the device and the `blocked` camp will be set to 1, then the control will come back to the original process that will call receive a message: at this point, the process is already blocked, so it will not be put on the waiting msg list. If the local timer does not fire before the blocking syscall then the process will be put in the waiting msg list and the `blocked` camp will be set to 1. Then the scheduler will be called and the SSI's `doIO` function will remove the process from the waiting msg list and it will put it in the list waiting for the device.  
When the device is finished and the interrupt is received the process waiting for the interrupt will be removed from the waiting device queue and put in the waiting message queue, then a message will be sent to the SSI that will send a message to the process and effectively unlock it.  
