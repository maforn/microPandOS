...
struct list_head blocked_pcbs[DEVINTNUM]
                             [DEVPERINT]; // processes waiting IO operation
...
pcb_t *true_ssi_pcb;
// set a memory address that is not used for anything else, as it is in kseg0
// and thus used for BIOS routines, not memory
